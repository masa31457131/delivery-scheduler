...
async function sendEmail({ to, subject, html }) {
  ...
  if (provider === 'gmail') {
    ...
    if (!gmail_user || !gmail_app_password) {
      console.log('[Email skipped - Gmail not configured]', subject); return { skipped: true };
    }
    try {
      const t = createGmailTransport(gmail_user, gmail_app_password);
      await t.sendMail({ from: `"納品スケジューラー" <${gmail_user}>`, to: toList, subject, html });
      console.log('[Gmail sent]', subject, '->', toList.join(', ')); return { success: true };
    } catch (e) { console.error('[Gmail error]', e.message); return { error: e.message }; }
  }

  if (provider === 'resend') {
    const resend = process.env.RESEND_API_KEY ? new Resend(process.env.RESEND_API_KEY) : null;
    if (!resend) { console.log('[Email skipped - no RESEND_API_KEY]', subject); return { skipped: true }; }
    const from = settings.from || '納品スケジューラー <onboarding@resend.dev>';
    try {
      const r = await resend.emails.send({ from, to: toList, subject, html });
      console.log('[Resend sent]', subject, '->', toList.join(', ')); return r;
...
async function initDB() {
  ...
  try {
    ...
    // project_name の NOT NULL 制約を削除（project_type に移行済みのため）
...
app.put('/api/settings/email', async (req, res) => {
  ...
  const value = JSON.stringify({
    ...
    gmail_app_password: (gmail_app_password && gmail_app_password !== '********')
      ? gmail_app_password : (existing.gmail_app_password || ''),
  });
...
app.post('/api/projects', async (req, res) => {
  ...
  if (!client_name || !project_type || !sales_rep)
    return res.status(400).json({ error: '必須項目が不足しています' });
  if (memo && memo.length > 30)
    return res.status(400).json({ error: '備考は30文字以内で入力してください' });

  const id = uuidv4();
  await pool.query(
    `INSERT INTO projects (id,client_name,project_type,sales_rep,memo,delivery_method,candidate_days,status)
...
app.put('/api/projects/:id', async (req, res) => {
  ...
  await pool.query(
    `UPDATE projects SET
      client_name=$1, project_type=$2, sales_rep=$3, memo=$4,
      status=$5, confirmed_date=$6, delivery_method=$7, candidate_days=$8,
      updated_at=NOW()
     WHERE id=$9`,
    [
      ...
      candidate_days ?? p.candidate_days,
      req.params.id,
    ]
  );
...
app.delete('/api/projects/:id/candidates/:candidateId', async (req, res) => {
  const { rows } = await pool.query(
    'SELECT * FROM schedule_candidates WHERE id=$1 AND project_id=$2',
