...
export default function DetailPage({ projectId, onBack, addToast, onRefresh }) {
  ...
  const handleConfirm = async (candidate) => {
    ...
    setBusy(true);
    try {
      const updated = await api.confirmProject(projectId, { confirmed_date: candidate.candidate_date, confirmed_time: candidate.candidate_time });
      setProject(updated);
      addToast('納品日を確定しました！');
      onRefresh();
    ...
    <>
      ...
      {project.candidates?.length > 0 && (
        <div className="card">
          ...
          {project.candidates.map(c => (
            <div key={c.id} className="confirm-card">
              <div>
                ...
                <div className="confirm-date">{formatDate(c.candidate_date)}{c.candidate_time && ` ${c.candidate_time}`}</div>
              </div>
              {isAdmin && (
          ...
          {!isAdmin && <div className="text-sub mt-8">管理者が候補日から確定します</div>}
        </div>
      )}
